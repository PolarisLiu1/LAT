...
class Qwen2VLGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if peft_config is not None:
            def find_all_linear_names(model, multimodal_keywords, input_single_image=False):
                cls = torch.nn.Linear
                lora_module_names = set()
                for name, module in model.named_modules():
                    # LoRA is not applied to the vision modules
                    if any(mm_keyword in name for mm_keyword in multimodal_keywords):
                        if isinstance(module, cls) and "patch_embed" not in name and ('proj' in name or 'qkv' in name) and input_single_image:
            ...
            target_modules = find_all_linear_names(model, self.vision_modules_keywords, single_image)
            print(f"Applying PEFT to the following modules: {target_modules}")
            peft_config.target_modules = target_modules # target_modules
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                # ---------------------------
                output_reward_func = reward_func(prompts=prompts, completions=completions, length=completion_mask.sum(1), **reward_kwargs)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        advantages = inputs["advantages"] # B, 1
